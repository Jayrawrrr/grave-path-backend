// backend/src/models/GardenC.js
import mongoose from 'mongoose';

const gardenCSchema = new mongoose.Schema({
...
gardenCSchema.pre('save', function(next) {
  this.updatedAt = Date.now;
  next();
});

// Create indexes for better performance
gardenCSchema.index({ type: 1 });
gardenCSchema.index({ row: 1, column: 1 });
gardenCSchema.index({ status: 1 });

const GardenC = mongoose.model('GardenC', gardenCSchema);
