// backend/src/models/GardenB.js
import mongoose from 'mongoose';

const gardenBSchema = new mongoose.Schema({
...
gardenBSchema.pre('save', function(next) {
  this.updatedAt = Date.now;
  next();
});

// Create indexes for better performance
gardenBSchema.index({ type: 1 });
gardenBSchema.index({ row: 1, column: 1 });
gardenBSchema.index({ status: 1 });

const GardenB = mongoose.model('GardenB', gardenBSchema);

export default GardenB;
