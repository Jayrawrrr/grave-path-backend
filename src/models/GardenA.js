// backend/src/models/GardenA.js
import mongoose from 'mongoose';

const gardenASchema = new mongoose.Schema({
...
gardenASchema.pre('save', function(next) {
  this.updatedAt = Date.now;
  next();
});

// Create indexes for better performance
gardenASchema.index({ type: 1 });
gardenASchema.index({ row: 1, column: 1 });
gardenASchema.index({ status: 1 });
gardenASchema.index({ centerCoordinates: '2dsphere' });

const GardenA = mongoose.model('GardenA', gardenASchema);

export default GardenA;
